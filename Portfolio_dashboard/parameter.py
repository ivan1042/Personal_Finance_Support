...
if submit:
    ...
    if len(stripped_weight) != len(stripped_stock):
        ...
        with col_1:
            ...
            st.metric("Yearly geo mean", f"{analyzed.bh_return_data[3]-1:.2%}")
            st.metric("Historical VaR", f"{analyzed.bh_static_risk_data[0] * capital:.2f}")
            st.metric("Parametric VaR", f"{analyzed.bh_static_risk_data[1] * capital:.2f}")
            st.metric("sharpe_ratio", f"{analyzed.bh_static_risk_data[2]:.2f}")
            st.metric("sortino_ratio", f"{analyzed.bh_static_risk_data[3]:.2f}")
            st.metric("max_drawdown", f"{analyzed.bh_static_risk_data[4] * capital:.2f}")
            st.metric("volatility", f"{analyzed.bh_static_risk_data[5]:.2f}")
        ...
        with col_2:
            ...
            st.subheader("return maxtrix")
            st.metric("Yearly geo mean", f"{analyzed.cw_return_data[3] - 1:.2%}")
            st.metric("Historical VaR", f"{analyzed.cw_static_risk_data[0] * capital:.2f}")
            st.metric("Parametric VaR", f"{analyzed.cw_static_risk_data[1] * capital:.2f}")
            st.metric("sharpe_ratio", f"{analyzed.cw_static_risk_data[2]:.2f}")
